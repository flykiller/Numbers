...
public:
    ...
    BN(int init_int = 0) {   // Создать новое BN
	    _n = 2 + (init_int >= RADIX || -(init_int) <= RADIX);
    ...
    BN & mod_to(BN const &r) {
        string old = to_string(10);
        if (r.is_zero()) throw "Divide by zero";
        int sign = _sign ^ r._sign;
        BN rem; //rem._b = NULL;
        divmod_internal(this, &r, &rem);
        //printf("mod_to(%s,%s)=(%s,%s)\n",old.c_str(), r.to_string(10).c_str(), to_string(10).c_str(), rem.to_string(10).c_str());
        ...
        free(_b);
        _b = rem._b; //
        //(mt *)malloc(rem._n * sizeof(mt));
        //memcpy(_b, rem._b, rem._n * sizeof(mt));
        _n = rem._n;
        _sign = rem._sign;
        rem._b = NULL;
        return *this;
